...
var app = new Vue({
 ...
 mounted:function(){
   setTimeout(function() {
      //console.log('аккуант', web3.eth.accounts, web3.eth.accounts.length);
 ...
 methods: {
    read_contract: function () {
      ...
      console.log("Гэрээний уншилт");
      if (window.location.hash && window.location.hash !== '#') {
        var address = window.location.hash.replace(/^#/,'');
        app.address = address;
        app.etherscanLink = "https://ropsten.etherscan.io/address/" + address;
        ...
        app.error = "Гэрээний хаяг байхгүй!";
      }
    }
  }
})
...
function verify_file(ev) {
  ...
  reader.onload = function(event) {
    ...
    console.log("Файлын хэш ", app.droppedHash);
    console.log("Блокчэйн дэх хэш ", app.hash);
